...
// Global server statistics and data structures
...
static io_thread_t io_workers[MAX_IO_THREADS];
static int num_cpu_cores;
static int num_worker_threads;
static int num_io_threads;

// Thread stack size (add to header if not defined)
...
static void setup_process_optimizations(void) {
    ...
    int ret = system("echo performance | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor > /dev/null 2>&1");
    if (ret != 0) {
        printf("Warning: Could not set CPU governor to performance mode\n");
    }
}
...
static void cleanup_and_exit(void) {
    ...
    for (int i = 0; i < num_worker_threads; i++) {
        if (workers[i].memory_pool && workers[i].memory_pool != MAP_FAILED) {
            munmap(workers[i].memory_pool, MEMORY_POOL_SIZE);
        }
        if (workers[i].event_fd > 0) {
...
int main(void) {
    ...
    for (int i = 0; i < num_worker_threads; i++) {
        ...
        if (workers[i].event_fd == -1) {
            perror("eventfd");
            exit(EXIT_FAILURE);
        }

        // Allocate per-thread memory pool using mmap
        ...
        atomic_store(&workers[i].local_requests, 0);
        atomic_store(&workers[i].local_bytes_sent, 0);
    ...
    signal(SIGINT, (__sighandler_t)cleanup_and_exit);
    signal(SIGTERM, (__sighandler_t)cleanup_and_exit);
    ...
    // This cleanup code will only be reached if the main loop exits
    cleanup_and_exit();
    return 0;
}
